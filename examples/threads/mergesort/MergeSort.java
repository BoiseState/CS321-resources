...
public class MergeSort
{
	...
	/**
	 * The parallel merge sort method. It splits array of elements in two halves
	 * to be sorted in parallel by the use of threads. Then both threads join at
	 * the merge to synchronize. Threads are generated recursively until we
	 * reach the cutoff specified by the user.
	 * 
	 * @param list
	 * @param start
	 * @param end
	...
	public void parallelMergeSort(int[] list, int start, int end, int level)
	{
		...
		// Thread that will call parallel merge on left list (this does not
		// start the
		// thread!)
		...
		// creates thread that will call parallel merge on right list (this does
		// not
		// start the thread)
	...
	/**
	 * Merge subArray list[start..midIndex] with list[midIndex+1...end] to
	 * create tje final sorted array.
	 * 
	 * @param list
	 * @param start
	 * @param midIndex
	...
	/**
	 * The driver that processes command line arguments and runs merge sort with
	 * or without threads.
	 * 
	 * @param args
	 */
	public static void main(String[] args)
	{
		...
		cutoffLevel = args.length == 3 ? Integer.parseInt(args[2])
				: DEFAULT_CUTOFF;
		...
		if (!helper.isSorted(array))
			...
			System.out.println("Mergesort with"
					+ (parallel ? " " + ((2 << cutoffLevel) - 1) + " " : "out ")
