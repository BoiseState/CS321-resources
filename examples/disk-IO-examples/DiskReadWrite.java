...
/**
 * This is a fairly complete example showing how to manipulate a binary search
 * tree stored on the disk as a external data structure in a binary file. The
 * layout of the binary search tree in the data file is as follows:
 * 
 ...
 * 
 * This example is for demonstration purposes and and has only gone through a
 * few smoke tests.
 * 
 * First create a binary search tree on disk with the command:
 * <tt> java DiskReadWrite --create </tt>
 * 
 * Then dump the tree to show we can read the tree from the disk. Use the
 * following command to print out the tree.
 * 
...
public class DiskReadWrite
{
	...
	/**
	 * Create a binary search tree on disk if it doesn't exist, otherwise read
	 * the metadata for a tree that already exists on disk.
	 * 
	 * @param fileName
	 * @throws IOException
	 */
	...
	private class Node
	{
		...
		private long parent, left, right; // these are byte offsets in the data
											// file
		/**
		 * Calculate the size of a node as stored on disk (in bytes). We will
		 * store boolean as 1 byte as its size is not defined in Java
		 */
		public static final int BYTES = Integer.BYTES + TreeObject.BYTES + 1
				+ 3 * Long.BYTES;
		...
		public Node(TreeObject key, boolean onDisk)
		{
			...
			if (onDisk) {
				...
				nextDiskAddress += nodeSize; // update pointer for next node on
												// disk
			}
		}
	}
	 ...
	 * 
	 * @param diskAddress
	 *                        the byte offset for the node in the data file
	...
	public Node diskRead(long diskAddress) throws IOException
	{
		...
		Node x = new Node(key, false); // the false flag is so that it isn't
	...
	/**
	 * Writes a node to the disk at the specified disk offset *in the Node
	 * object).
	 * 
	 * @param x
	 *              the Node to write
	...
	/**
	 * Cleanup at the end. Writes the root node and metadata and closes the data
	 * file.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Insert a new object into the external binary search tree. This follows
	 * the algorithm from the book at page 321 of CLRS Introduction to
	 * Algorithms book, except for adding disk read/writes.
	 * 
	 * @param obj
	 *                the object t insert
	 * @throws IOException
	...
	/**
	 * Recursive inorder traversal. The usual procedure except for the inclusion
	 * of disk reads.
	 * 
	 * @param x
	 *              the starting node
	 ...
	 * 
	 * @param file
	 *                 the data file where the tree will be stored
	 * @throws IOException
	 */
	...
	/**
	 * Dump the binary search tree (inorder traversal) from the give binary data
	 * file.
	 * 
	 * @param file
	 * @throws IOException
	...
	/**
	 * The main program that takes a command line argument to either create or
	 * to dump an external binary search tree.
	 * 
	 * @param args
	 * @throws IOException
	 */
	public static void main(String[] args) throws IOException
	{
		...
		if (args[0].equals("--create")) {
			createTree(file);
			System.out.println(
					"DiskReadWrite: created a binary search tree in bst.data binary file");
		} else if (args[0].equals("--dump")) {
			dumpTree(file);
			System.out.println(
					"DiskReadWrite: dumping a binary search tree from the file bst.data");
		} else {
